import { PrismaClient, Ticket, TicketCluster, TicketClusterMember, TicketPriority } from '@prisma/client';
import { ILogger } from '@pjtaudirabot/core';

export interface ClusteringOptions {
  windowMinutes?: number;        // Time window to look for similar tickets (default 60)
  similarityThreshold?: number;  // Min similarity to cluster (default 0.75 = 75%)
  minClusterSize?: number;       // Min tickets to auto-create cluster (default 2)
  maxMembersPerCluster?: number; // Max members per cluster (default 50)
}

export interface SimilarityFactors {
  location: number;        // weight 0.3
  customer: number;        // weight 0.2
  service: number;         // weight 0.2
  problem: number;         // weight 0.2 (string similarity)
  priority: number;        // weight 0.1
}
...
export class TicketClusteringService {
  ...
  private readonly options: Required<ClusteringOptions>;
   ...
   */
  async clusterNewTicket(ticket: Ticket): Promise<{ clusterId?: string; clusterNumber?: string; memberCount: number; created: boolean }> {
    try {
      const since = new Date(Date.now() - this.options.windowMinutes * 60_000);
      ...
      this.logger.error('Clustering failed', error as Error, { ticketId: ticket.id });
      return { memberCount: 1, created: false };
    }
  }
   ...
   */
  async cascadeResolution(ticketId: string, rootCause: string, solution: string, changedById?: string): Promise<void> {
    try {
      ...
      this.logger.error('Cascade resolution failed', error as Error, { ticketId });
      throw error;
    }
  }
   ...
   */
  private getMatchingFactors(ticket1: Ticket, ticket2: Ticket): string[] {
    const factors: string[] = [];
    if (ticket1.location && ticket1.location === ticket2.location) factors.push('location');
    if (ticket1.customer && ticket1.customer === ticket2.customer) factors.push('customer');
    if (ticket1.service && ticket1.service === ticket2.service) factors.push('service');
    if (this.stringSimilarity(ticket1.problem, ticket2.problem) > 0.7) factors.push('problem');
    return factors;
  }
   ...
   * Exact match scoring
   */
  private scoreMatch(value1: string | null, value2: string | null, weight: number): number {
    if (!value1 || !value2) return 0;
    if (value1.toLocaleLowerCase() === value2.toLocaleLowerCase()) return weight;
    return 0;
  }
   ...
   */
  private priorityMatch(priority1: TicketPriority, priority2: TicketPriority): number {
  ...
  private async createCluster(
    masterTicket: Ticket,
    scored: Array<{ ticket: Ticket; similarity: number; factors: string[] }>,
  ): Promise<TicketCluster & { members: TicketClusterMember[] }> {
    ...
    const impactScore = Math.min(100,
      ...
      (priorityWeights[masterTicket.priority as TicketPriority] ?? 0),
    );
   ...
   */
  private async addMemberToCluster(
    clusterId: string,
    ticket: Ticket,
    similarity: number,
    factors: string[],
